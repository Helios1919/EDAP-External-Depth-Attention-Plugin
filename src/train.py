...
if args.freeze_lm_head:
    ...
    # Pure identity init: the bottleneck is a no-op at step 0, so the model
    # starts exactly at original Qwen behaviour (matching EDAP's zero-init W_O
    # "identity start" philosophy). A random init would scramble the residual
    # stream and inflate the initial loss, masking EDAP's learning signal.
    # NOTE: even a "tiny" perturbation like 0.02*randn is scaled by √3584≈60
    # through the d×d matmul, producing noise with std ≈1.2×‖hidden‖ — as bad
    # as a random init. Use exact identity.
